 ...
 (function($) {
		...
		*/
		...
		function view(map) {
			...
			if(params.mode2d) {
				...
				//console.log(debut_x + " - " + fin_x + ' / ' + cursor_posx);
				if(debut_x < 1) {
		...
		**/
		...
		function getDecZZone(x, y, z, zone, zonelength){
			...
			var supports = getItemZone((x|0), (y|0), z, zone, zonelength);
			if(!!supports){
				var dummy = 0;
				var dummy_id = 0;
				...
				var ref = getItem(supports[dummy_id].x, supports[dummy_id].y, 0)[0];
		...
		function moveCursor(titleset) {
			...
			var posy = infos[1];
			var decZ = 0;
			if(typeof(getMaxAlt)!='undefined' && items[Choix]['type']!='sols'){
				if(Choix !=0){
					var cZone = JSON.parse(items[Choix].zone);
					var cZl = cZone.length;
					if(cZl>1){
						cursorAlti = getMaxAltZone(posx, posy, cZone, cZl);
						decZ = getDecZZone(posx, posy, cursorAlti, cZone, cZl);
					}
					else{
						cursorAlti = getMaxAlt(posx, posy);
			...
			if(typeof(myCursorDelay)!='undefined'){
				myCursorDelay(posx, posy);
			}
			...
			for(var i = 0; i < boucleCond; i++) {
				...
				if(params.zoom!=1 && theBrowser=='msie') {
					$('#' + id).css({
						...
						'background-position':'center center',
						'background-repeat':'no-repeat',
						'-ms-filter':"progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + params.cursorPF + "', sizingMethod='scale')",
						'filter':"progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + params.cursorPF + "', sizingMethod='scale')",
		...
		**/
		...
		function _addObject(x, y, z, sprite, decx, decy, vid, titre, bulle, id) {
			...
			var dZ = 0;
			if(typeof(items[vid])!='undefined'){
				var itzone = JSON.parse(items[vid].zone);
