...
if ($tid)
	$result = $db->query('SELECT f.id, f.forum_name, f.moderators, f.redirect_url, f.clan_id, f.clan_mode, fp.post_replies, fp.post_topics, t.subject, t.closed, s.user_id AS is_subscribed, tc.cat_name FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') LEFT JOIN '.$db->prefix.'topic_subscriptions AS s ON (t.id=s.topic_id AND s.user_id='.$pun_user['id'].') JOIN '.$db->prefix.'categories AS tc ON tc.id = f.cat_id WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$tid) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
else
	$result = $db->query('SELECT f.id, f.forum_name, f.moderators, f.redirect_url, f.clan_id, f.clan_mode, fp.post_replies, fp.post_topics, tc.cat_name FROM '.$db->prefix.'forums AS f LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') JOIN '.$db->prefix.'categories AS tc ON tc.id = f.cat_id WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.id='.$fid) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
...
// Si c'est un forum de clan fermé ou caché, on vérifie que le joueur a le droit de poster
if ($cur_posting['clan_id'] != null && $pun_user['g_id'] != PUN_ADMIN && (($cur_posting['clan_mode'] == 2 && ! in_array($pun_user['id'], $mods_array)) || $cur_posting['clan_mode'] == 3))
{
	$result_tmp = $db->query('SELECT 1 FROM politiciens WHERE clan_id = '.$cur_posting['clan_id'].' AND joueur_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());

	// Pas membre, peut-être chef ?
	if ($result_tmp->num_rows == 0)
	{
		$result_tmp = $db->query('SELECT 1 FROM clans WHERE id = '.$cur_posting['clan_id'].' AND chef_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());
...
if (preg_match('#\[Rôle Play\]#', $cur_posting['cat_name']))
	$zone_rp = true;
else
	$zone_rp = false;
...
if (isset($_POST['form_sent']))
{
	...
	if ($zone_rp)
	{
		$result = $db->query("SELECT valeur FROM `config` WHERE `cle` = 'mots_sensible_hrp'") or error('Unable to fetch config tobozon', __FILE__, __LINE__, $db->error());
		$mots_hrp = $db->fetch_assoc($result);

		$array_mots_hrp = explode(',', $mots_hrp['valeur']);

		foreach ($array_mots_hrp as $mot)
		{
			if (stripos($orig_message, $mot) !== false)
				$hrp_sensible[] = $mot;
		}
	}
	...
	$verif_hrp = isset($_POST['verif_hrp']) ? '1' : '0';
	...
	if ((empty($hrp_sensible) || $verif_hrp == 1) && empty($errors) && !isset($_POST['preview']))
	{
		...
		if ($tid)
		{
			...
			if ($pun_config['o_topic_subscriptions'] == '1')
			{
				...
				// Get any subscribed users that should be notified
				$result = $db->query('SELECT u.id, u.email, u.notify_with_post, u.language FROM '.$db->prefix.'users AS u INNER JOIN '.$db->prefix.'topic_subscriptions AS s ON u.id=s.user_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id='.$cur_posting['id'].' AND fp.group_id=u.group_id) LEFT JOIN '.$db->prefix.'online AS o ON u.id=o.user_id WHERE COALESCE(o.logged, u.last_visit)>'.$previous_post_time.' AND (fp.read_forum IS NULL OR fp.read_forum=1) AND s.topic_id='.$tid.' AND u.id!='.$pun_user['id']) or error('Unable to fetch subscription info', __FILE__, __LINE__, $db->error());
				if ($db->num_rows($result))
				{
		...
		else if ($fid)
		{
			...
			if (!$pun_user['is_guest'])
			{
				...
				$db->query('INSERT INTO '.$db->prefix.'posts (poster, poster_id, poster_ip, message, hide_smilies, posted, topic_id) VALUES(\''.$db->escape($username).'\', '.$pun_user['id'].', "", \''.$db->escape($message).'\', '.$hide_smilies.', '.$now.', '.$new_tid.')') or error('Unable to create post', __FILE__, __LINE__, $db->error());
			}
			else
			{
				// Create the post ("topic post")
				$email_sql = ($pun_config['p_force_guest_email'] == '1' || $email != '') ? '\''.$db->escape($email).'\'' : 'NULL';
				$db->query('INSERT INTO '.$db->prefix.'posts (poster, poster_ip, poster_email, message, hide_smilies, posted, topic_id) VALUES(\''.$db->escape($username).'\', "", '.$email_sql.', \''.$db->escape($message).'\', '.$hide_smilies.', '.$now.', '.$new_tid.')') or error('Unable to create post', __FILE__, __LINE__, $db->error());
			}
			$new_pid = $db->insert_id();
			...
			{
				// Get any subscribed users that should be notified
				$result = $db->query('SELECT u.id, u.email, u.notify_with_post, u.language FROM '.$db->prefix.'users AS u INNER JOIN '.$db->prefix.'forum_subscriptions AS s ON u.id=s.user_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id='.$cur_posting['id'].' AND fp.group_id=u.group_id) WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND s.forum_id='.$cur_posting['id'].' AND u.id!='.$pun_user['id']) or error('Unable to fetch subscription info', __FILE__, __LINE__, $db->error());
				if ($db->num_rows($result))
				{
					require_once PUN_ROOT.'include/email.php';
...
elseif (!empty($hrp_sensible))
{

?>
<div id="posterror" class="block">
	<h2><span>Mots sensibles HRP détectés</span></h2>
	<div class="box">
		<div class="inbox error-info">
			<p>Des mots sensiblement HRP ont été détectés dans le message, vérifiez la bonne utilisation de ceux-ci avant de renvoyer votre message :</p>
			<ul class="error-list">
<?php

	foreach ($hrp_sensible as $cur_hrp)
		echo "\t\t\t\t".'<li><strong>'.$cur_hrp.'</strong></li>'."\n";
?>
			</ul>
		</div>
	</div>
...
if (isset($verif_hrp))
	echo '<input type="hidden" name="verif_hrp" value="1" />';
?>
			</div>
