...
$result = $db->query('SELECT f.id AS fid, f.forum_name, f.moderators, f.redirect_url, f.clan_id, f.clan_mode, fp.post_replies, fp.post_topics, t.id AS tid, t.subject, t.posted, t.first_post_id, t.sticky, t.closed, p.poster, p.poster_id, p.posted, p.message, p.hide_smilies, tc.cat_name FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'topics AS t ON t.id=p.topic_id INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') JOIN '.$db->prefix.'categories AS tc ON tc.id = f.cat_id WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND p.id='.$id) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
...
if ((($pun_user['g_edit_posts'] == '0' && $cur_post['posted'] < time() - 300) ||
	$cur_post['poster_id'] != $pun_user['id'] ||
	$cur_post['closed'] == '1') &&
	!$is_admmod)
	message($lang_common['No permission'], false, '403 Forbidden');

// Si c'est un forum de clan caché, on vérifie que le joueur a le droit de modifier
if ($cur_post['clan_id'] != null && $pun_user['g_id'] != PUN_ADMIN && $cur_post['clan_mode'] == 3)
{
	$result_tmp = $db->query('SELECT 1 FROM politiciens WHERE clan_id = '.$cur_post['clan_id'].' AND joueur_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());

	// Pas membre, peut-être chef ?
	if ($result_tmp->num_rows == 0)
	{
		$result_tmp = $db->query('SELECT 1 FROM clans WHERE id = '.$cur_post['clan_id'].' AND chef_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());
...
if (preg_match('#\[Rôle Play\]#', $cur_post['cat_name']))
	$zone_rp = true;
else
	$zone_rp = false;
...
if (isset($_POST['form_sent']))
{
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		if ($pun_user['g_id'] == PUN_ADMIN)
			$edited_sql = (!isset($_POST['silent'])) ? ', edited='.time().', edited_by=\''.$db->escape($pun_user['username']).'\'' : '';
		else
		...
		// On enregistre la modération dans l'historique
		if ($cur_post['poster_id'] != $pun_user['id'])
			$db->query('INSERT INTO tobozon_log_moderation VALUES("", '.$cur_post['poster_id'].', "'.$db->escape($cur_post['message']).'", '.$pun_user['id'].', "'.$db->escape($message).'", NOW(), '.$id.')') or error('Unable to log post moderation', __FILE__, __LINE__, $db->error());
