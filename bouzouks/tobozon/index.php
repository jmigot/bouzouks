...
$result = $db->query('SELECT c.id AS cid, c.cat_name, f.id AS fid, f.forum_name, f.forum_desc, f.redirect_url, f.moderators, f.num_topics, f.num_posts, f.last_post, f.last_post_id, f.last_poster, f.clan_id, f.clan_mode FROM '.$db->prefix.'categories AS c INNER JOIN '.$db->prefix.'forums AS f ON c.id=f.cat_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE fp.read_forum IS NULL OR fp.read_forum=1 ORDER BY c.disp_position, c.id, f.disp_position', true) or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());

$cur_category = 0;
$cat_count = 0;
$forum_count = 0;
$clan_modes = array(1 => 'ouvert', 2 => 'ferme', 3 => 'cache');
$clan_modes_title = array(1 => 'Clan ouvert', 2 => 'Clan fermé', 3 => 'Clan caché');
...
<?php
	...
	// Si c'est un forum clan, on affiche l'image correspondante aux droits (ouvert, fermé, caché)
	if ($cur_forum['clan_id'] != null)
		$forum_field .= '<img src="../webroot/images/tobozon/clan_'.$clan_modes[$cur_forum['clan_mode']].'.png" alt="'.$clan_modes_title[$cur_forum['clan_mode']].'" title="'.$clan_modes_title[$cur_forum['clan_mode']].'" width="15px" style="float: left; margin-right: 5px;">';
	...
	if ($cur_forum['forum_desc'] != '')
		$forum_field .= "\n\t\t\t\t\t\t\t\t".'<div class="forumdesc">'.pun_htmlspecialchars($cur_forum['forum_desc']).'</div>';
	...
	if ($cur_forum['moderators'] != '')
	{
		...
		foreach ($mods_array as $mod_username => $mod_id)
		{
			if ($pun_user['g_view_users'] == '1')
				$moderators[] = '<a href="http://www.bouzouks.net/communaute/profil/'.$mod_id.'">'.pun_htmlspecialchars($mod_username).'</a>';
			else
