	...
	if (!empty($search_ids))
	{
		...
		while ($row = $db->fetch_assoc($result))
		{
			...
			if ($row['clan_id'] != null && $pun_user['g_id'] != PUN_ADMIN)
			{
				...
				if ($row['clan_mode'] == 3)
				{
					$result_tmp = $db->query('SELECT 1 FROM politiciens WHERE clan_id = '.$row['clan_id'].' AND joueur_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());

					// Pas membre, peut-être chef ?
					if ($result_tmp->num_rows == 0)
					{
						$result_tmp = $db->query('SELECT 1 FROM clans WHERE id = '.$row['clan_id'].' AND chef_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());
...
$result = $db->query('SELECT c.id AS cid, c.cat_name, f.id AS fid, f.forum_name, f.redirect_url, f.clan_id, f.clan_mode FROM '.$db->prefix.'categories AS c INNER JOIN '.$db->prefix.'forums AS f ON c.id=f.cat_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.redirect_url IS NULL ORDER BY c.disp_position, c.id, f.disp_position', true) or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());
...
if ($pun_config['o_search_all_forums'] == '1' || $pun_user['is_admmod'])
{
	...
	while ($cur_forum = $db->fetch_assoc($result))
	{
		...
		// Si c'est un forum de clan, on vérifie que le joueur a le droit de le voir
		if ($cur_forum['clan_id'] != null && $pun_user['g_id'] != PUN_ADMIN)
		{
			// Les invités n'ont pas le droit de lecture pour ne pas polluer l'affichage
			if ($pun_user['is_guest'])
				continue;
			...
			// Si le clan est caché, il n'apparait pas sauf si on est chef ou membre
			if ($cur_forum['clan_mode'] == 3)
			{
				$result_tmp = $db->query('SELECT 1 FROM politiciens WHERE clan_id = '.$cur_forum['clan_id'].' AND joueur_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());

				// Pas membre, peut-être chef ?
				if ($result_tmp->num_rows == 0)
				{
					$result_tmp = $db->query('SELECT 1 FROM clans WHERE id = '.$cur_forum['clan_id'].' AND chef_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());
