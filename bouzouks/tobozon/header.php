...
else
{
	...
	if ($pun_user['is_admmod'])
	{
		...
		{
			// Requete largement modifiée pour les clans
			$admin = $pun_user['g_id'] == PUN_ADMIN ? 1 : 0;
			$result_header = $db->query('SELECT 1 FROM '.$db->prefix.'reports WHERE zapped IS NULL AND ('.$admin.'. OR forum_id NOT IN (
						SELECT id FROM tobozon_forums tf WHERE tf.clan_id IS NOT NULL AND tf.clan_mode = 3 AND NOT EXISTS (
							SELECT 1 FROM politiciens p WHERE p.clan_id = tf.clan_id AND p.joueur_id = '.$pun_user['id'].'
								UNION
							SELECT 1 FROM clans c WHERE c.id = tf.clan_id AND c.chef_id = '.$pun_user['id'].'
						)))') or error('Unable to fetch reports info', __FILE__, __LINE__, $db->error());
