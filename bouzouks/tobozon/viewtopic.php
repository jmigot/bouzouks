...
if (!$pun_user['is_guest'])
	$result = $db->query('SELECT t.subject, t.closed, t.num_replies, t.sticky, t.first_post_id, f.id AS forum_id, f.forum_name, f.moderators, f.clan_id, f.clan_mode, fp.post_replies, s.user_id AS is_subscribed, tc.cat_name FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'topic_subscriptions AS s ON (t.id=s.topic_id AND s.user_id='.$pun_user['id'].') LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') JOIN '.$db->prefix.'categories AS tc ON tc.id = f.cat_id WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$id.' AND t.moved_to IS NULL') or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
else
	$result = $db->query('SELECT t.subject, t.closed, t.num_replies, t.sticky, t.first_post_id, f.id AS forum_id, f.forum_name, f.moderators, fp.post_replies, f.clan_id, f.clan_mode, 0 AS is_subscribed, tc.cat_name FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') JOIN '.$db->prefix.'categories AS tc ON tc.id = f.cat_id WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.id='.$id.' AND t.moved_to IS NULL') or error('Unable to fetch topic info', __FILE__, __LINE__, $db->error());
...
// Si c'est un forum de clan, on vérifie que le joueur a le droit de le voir
if ($cur_topic['clan_id'] != null && $pun_user['g_id'] != PUN_ADMIN)
{
	// Les invités n'ont pas le droit de lecture pour ne pas polluer l'affichage
	if ($pun_user['is_guest'])
		message($lang_common['Bad request'], false, '404 Not Found');

	// Si le clan est caché, il n'apparait pas sauf si on est chef ou membre
	if ($cur_topic['clan_mode'] == 3)
	{
		$result_tmp = $db->query('SELECT 1 FROM politiciens WHERE clan_id = '.$cur_topic['clan_id'].' AND joueur_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());

		// Pas membre, peut-être chef ?
		if ($result_tmp->num_rows == 0)
		{
			$result_tmp = $db->query('SELECT 1 FROM clans WHERE id = '.$cur_topic['clan_id'].' AND chef_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());
...
if (preg_match('#\[Rôle Play\]#', $cur_topic['cat_name']))
	$zone_rp = true;
else
	$zone_rp = false;
...
$result = $db->query('select post_id FROM '.$db->prefix.'likes WHERE joueur_id='.(int)$pun_user['id']) or error('Unable to fetch likes', __FILE__, __LINE__, $db->error());;
$likes_posts_ids = array();
for ($i = 0;$cur_like_post_id = $db->result($result, $i);$i++)
	$likes_posts_ids[] = $cur_like_post_id;

if (empty($post_ids))
	error('The post table and topic table seem to be out of sync!', __FILE__, __LINE__);

// Retrieve the posts (and their respective poster/online status)
$result = $db->query('SELECT u.email, u.title, u.url, u.location, u.signature, u.email_setting, u.num_posts, u.registered, u.admin_note, p.id, p.poster AS username, p.poster_id, p.poster_ip, p.poster_email, p.message, p.hide_smilies, p.posted, p.edited, p.edited_by, p.likes, g.g_id, g.g_user_title, o.user_id AS is_online FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'users AS u ON u.id=p.poster_id INNER JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id LEFT JOIN '.$db->prefix.'online AS o ON (o.user_id=u.id AND o.user_id!=1 AND o.idle=0) WHERE p.id IN ('.implode(',', $post_ids).') ORDER BY p.id', true) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
while ($cur_post = $db->fetch_assoc($result))
{
	...
	// If the poster is a registered user (on exclut les 15 premiers robots)
	if ($cur_post['poster_id'] > 15)
	{
		if ($pun_user['g_view_users'] == '1')
			$username = '<a href="http://www.bouzouks.net/communaute/profil/'.$cur_post['poster_id'].'">'.pun_htmlspecialchars($cur_post['username']).'</a>';
		else
		...
		$is_online = ($cur_post['is_online'] == $cur_post['poster_id']) ? '<span style="color: green"><strong>'.$lang_topic['Online'].'</strong></span>' : '<span>'.$lang_topic['Offline'].'</span>';
	...
	if (!$is_admmod)
	{
		if (!$pun_user['is_guest'])
		{
			if (in_array($cur_post['id'], $likes_posts_ids))
				$post_actions[] = '<li class="postdislike"><span><a href="#'.$cur_post['id'].'">'.$lang_topic['Dislike'].'</a></span></li>';
			else
				$post_actions[] = '<li class="postlike"><span><a href="#'.$cur_post['id'].'">'.$lang_topic['Like'].'</a></span></li>';
		...
		if ($cur_topic['closed'] == '0')
		{
			if ($cur_post['poster_id'] == $pun_user['id'])
			{
				...
				if ($pun_user['g_edit_posts'] == '1' || $cur_post['posted'] > time() - 300)
...
<div id="p<?php echo $cur_post['id'] ?>" class="blockpost<?php echo ($post_count % 2 == 0) ? ' roweven' : ' rowodd' ?><?php if ($cur_post['id'] == $cur_topic['first_post_id']) echo ' firstpost'; ?><?php if ($post_count == 1) echo ' blockpost1'; ?>">
	<h2><span><span class="conr">#<?php echo ($start_from + $post_count) ?></span> <span class="fl-gauche"><a href="viewtopic.php?pid=<?php echo $cur_post['id'].'#p'.$cur_post['id'] ?>"><?php echo format_time($cur_post['posted']) ?></a></span><span class="like<?= ($cur_post['likes'] == 0) ? ' invisible' : '' ?>"><?= $cur_post['likes'] ?></span></span></h2>
	<div class="box">
		<div class="inbox">
			<div class="postbody">
				<div class="postleft">
...
if ($quickpost)
{
	...
	if ($cur_topic['clan_id'] != null && $pun_user['g_id'] != PUN_ADMIN && (($cur_topic['clan_mode'] == 2 && ! in_array($pun_user['id'], unserialize($cur_topic['moderators']))) || $cur_topic['clan_mode'] == 3))
	{
		$result_tmp = $db->query('SELECT 1 FROM politiciens WHERE clan_id = '.$cur_topic['clan_id'].' AND joueur_id = '.$pun_user['id'], true) or error('Unable to fetch clan members list', __FILE__, __LINE__, $db->error());
...
else if ($pun_user['group_id'] == 12)
{
?>

<p class="interdit_ecriture rouge">Tu as été interdit d'écriture sur le Tobozon par un modérateur/administrateur</p>
