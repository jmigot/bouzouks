...
if (isset($_POST['zap_id']))
{
	...
	$result = $db->query('SELECT r.zapped, tf.moderators FROM '.$db->prefix.'reports r JOIN tobozon_forums tf ON tf.id = r.forum_id WHERE r.id='.$zap_id) or error('Unable to fetch report info', __FILE__, __LINE__, $db->error());
	$zapped = $db->fetch_assoc($result);

	// Le joueur doit être modérateur du forum pour résoudre le signalement (solution pour intégrer les chefs de clan ici)
	$moderateurs = unserialize($zapped['moderators']);

	if ($pun_user['g_id'] != PUN_ADMIN && is_array($moderators) && ! in_array($pun_user['id'], $moderateurs))
		message($lang_common['No permission'], false, '403 Forbidden');

	if ($zapped['zapped'] == null)
...
$result = $db->query('SELECT r.id, r.topic_id, r.forum_id, r.reported_by, r.created, r.message, p.id AS pid, t.subject, f.forum_name, f.moderators, u.username AS reporter FROM '.$db->prefix.'reports AS r LEFT JOIN '.$db->prefix.'posts AS p ON r.post_id=p.id LEFT JOIN '.$db->prefix.'topics AS t ON r.topic_id=t.id LEFT JOIN '.$db->prefix.'forums AS f ON r.forum_id=f.id LEFT JOIN '.$db->prefix.'users AS u ON r.reported_by=u.id WHERE r.zapped IS NULL ORDER BY created DESC') or error('Unable to fetch report list', __FILE__, __LINE__, $db->error());

if ($db->num_rows($result))
{
	// Ajouté pour les clans
	$nb_traites = 0;

	while ($cur_report = $db->fetch_assoc($result))
	{
		// Le joueur doit être modérateur du forum pour voir le signalement (solution pour intégrer les chefs de clan ici)
		$moderateurs = unserialize($cur_report['moderators']);

		if ($pun_user['g_id'] != PUN_ADMIN && is_array($moderateurs) && ! in_array($pun_user['id'], $moderateurs))
			continue;
...
$result = $db->query('SELECT r.id, r.topic_id, r.forum_id, r.reported_by, r.message, r.zapped, r.zapped_by AS zapped_by_id, p.id AS pid, t.subject, f.forum_name, f.moderators, u.username AS reporter, u2.username AS zapped_by FROM '.$db->prefix.'reports AS r LEFT JOIN '.$db->prefix.'posts AS p ON r.post_id=p.id LEFT JOIN '.$db->prefix.'topics AS t ON r.topic_id=t.id LEFT JOIN '.$db->prefix.'forums AS f ON r.forum_id=f.id LEFT JOIN '.$db->prefix.'users AS u ON r.reported_by=u.id LEFT JOIN '.$db->prefix.'users AS u2 ON r.zapped_by=u2.id WHERE r.zapped IS NOT NULL ORDER BY zapped DESC LIMIT 10') or error('Unable to fetch report list', __FILE__, __LINE__, $db->error());
